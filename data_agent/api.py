...
def health(e:Engine=Depends(engine)):
    return {'status':'ok','mode':e.settings.mode,'transport':e.settings.transport,'as_of':e.settings.as_of,
            ...
            'embedding':e.retriever.backend,'rerank':e.retriever.rerank_backend,'schema_version':SCHEMA_VERSION,'watermark':e.gateway.settings.data_dir.joinpath('watermark.json').exists()}
...
async def ask(body:Ask,owner=Depends(identity),e:Engine=Depends(engine)):
    return await e.ask(body.question,body.session_id,body.request_id,owner,body.memory_ids)
...
def save_memory(body:SaveMemory,owner=Depends(identity),e:Engine=Depends(engine)):
    try:return e.memory.save(owner,**body.model_dump())
    except ValueError as exc:raise HTTPException(422,str(exc)) from exc

@app.delete('/api/memories/{memory_id}')
def delete_memory(memory_id:str,owner=Depends(identity),e:Engine=Depends(engine)):
    e.memory.delete(owner,memory_id);return {'deleted':True}

@app.get('/api/sessions/{session_id}/context')
def session_context(session_id:str,owner=Depends(identity),e:Engine=Depends(engine)):
